...
def jacobi(a, accuracy=0.01):  # Jacobi method

    def maxElem(a: ndarray):  # Find largest off-diag. element a[max_i,max_j]
        n = len(a)
        aMax = 0
        max_i = max_j = -1
        for i in range(n - 1):
    ...
    def rotate(matrix, i, j, n):  # Rotate to make a[max_i,max_j] = 0
    ...
    while 1:  # Jacobi rotation loop
        max_element, max_i, max_j = maxElem(matrix_copy)
        ...
        iterations += 1
        transformation_matrix = rotate(matrix_copy, max_i, max_j, n)
